...
# There is a double indexation, so let's remove the first column using dplyr
library(dplyr)
adult = select(adult, -X) ## [ Note: fnlwgt: final sampling weight]
head(adult)  #Now we re good to go
str(adult)

# But some variables are character whereas they should be factors
...
# Now let's check the histogram of hours worked per week
...
# Let's rename the country column to region column to better reflect the factor levels
adult = rename(adult, region = country)
head(adult)

# Let's now check the barplot of region by income class 
